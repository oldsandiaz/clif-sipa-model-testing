...
```{r table1}
vars_of_interest <- c("norepinephrine", "spo2", "gcs_total", "fio2_approx", 
                      "po2_arterial", "platelet_count", "dobutamine", 
                      "bilirubin_total", "creatinine", "vasopressin", 
                      "epinephrine", "phenylephrine", "dopamine", 
                      "milrinone", "angiotensin", "map")

# Get n for each mortality group
group_counts <- final_data %>%
  filter(variable_name %in% vars_of_interest, !is.na(value)) %>%
  distinct(hospitalization_id, in_hospital_mortality) %>%
  count(in_hospital_mortality) %>%
  deframe()

# Summary stats
summary_stats <- final_data %>%
  filter(variable_name %in% vars_of_interest, !is.na(value)) %>%
  group_by(variable_name, in_hospital_mortality) %>%
  summarise(
  ...
  mutate(
    summary = paste0(mean, " (", sd, "), median: ", median)
  ) %>%
  select(variable_name, in_hospital_mortality, summary) %>%
  pivot_wider(
    names_from = in_hospital_mortality,
    values_from = summary,
...
table1 <- summary_stats %>%
  rename(`Variable` = variable_name,
         !!paste0("Survivors (n = ", group_counts["0"], ")") := mortality_0,
         !!paste0("Non-Survivors (n = ", group_counts["1"], ")") := mortality_1) %>%
  gt(rowname_col = "Variable") %>%
  tab_header(
    title = "Table 1: Summary Statistics by In-Hospital Mortality"
  ) %>%
  fmt_missing(
    columns = everything(),
    missing_text = "—"
